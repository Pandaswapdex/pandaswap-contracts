...
contract RewarderMock is IRewarder {
    ...
    function onBambooReward (uint256, address user, uint256 bambooAmount) override external {
        uint256 pendingReward = bambooAmount.mul(rewardMultiplier) / REWARD_TOKEN_DIVISOR;
    ...
    function pendingTokens(uint256 pid, address user, uint256 bambooAmount) override external returns (IERC20[] memory rewardTokens, uint256[] memory rewardAmounts) {
        ...
        _rewardAmounts[0] = bambooAmount.mul(rewardMultiplier) / REWARD_TOKEN_DIVISOR;
